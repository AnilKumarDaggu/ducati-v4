...
export function LoadingOverlay({ progress, done }: { progress: number; done: boolean }) {
  return (
    <div
      className={`pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center bg-bianco transition-opacity duration-700 ${
        done ? 'opacity-0' : 'opacity-100'
      }`}
      aria-hidden={done}
    >
      <p className="display-caps text-[11px] tracking-[0.28em] text-rosso">Ducati</p>
      <p className="display mb-6 text-3xl text-nero">Panigale V4</p>
      <div className="h-px w-56 overflow-hidden bg-grigio-200">
        <div
