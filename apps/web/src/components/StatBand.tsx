  ...
  return (
    <section className="panel z-10 shrink-0 border-t" aria-label="Motorcycle specifications">
      <div className="flex items-stretch gap-px overflow-x-auto">
        <div className="flex min-w-56 flex-col justify-center border-r border-white/5 px-5 py-3">
          <p className="display-caps text-[10px] tracking-[0.18em] text-rosso">Ducati</p>
          <p className="display text-sm text-white">Panigale V4</p>
        ...
        {STATS.map((s) => (
          <div key={s.label} className="flex flex-1 flex-col justify-center px-4 py-3">
            <p className="display-caps text-[9px] tracking-[0.16em] text-grigio-500">{s.label}</p>
            <p className="flex items-baseline gap-1">
              <span className="data text-xl font-semibold text-white">{s.value}</span>
              <span className="data text-[11px] text-grigio-400">{s.unit}</span>
            </p>
          </div>
        ))}
      </div>
